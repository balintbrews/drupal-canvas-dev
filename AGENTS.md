...
- Monorepo packages (CLI, workbench, Vite plugin, extensions, and others) in `web/modules/contrib/canvas/packages/*`.
...
- Never run destructive Drush or site commands (for example, reinstall or DB-destructive operations) without explicit user approval.
...
When code changes are made, run targeted checks first, then broader checks where appropriate.
...
- If covered by Cypress component tests, run only impacted specs: `ddev cy --component --spec <relative-spec-path>`.
...
- If impacted, run targeted specs only: `ddev playwright --spec <relative-spec-path>`.
...
- Run coding standards checks and fixes after backend changes: `ddev phpcs` (optionally scoped to a relative path).

### CLI package

- No standardized test workflow is documented yet.
- Do not guess broad test commands; ask or use only task-specific verified commands.
...
- `canvas-ui-code-editor` for code editor lifecycle, state model, and high-risk touchpoints.
- `canvas-test-selector` for selecting the minimum safe test commands by changed paths.
