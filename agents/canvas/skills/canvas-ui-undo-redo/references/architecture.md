...
3. UI coordination in `src/features/ui/uiSlice.ts` records order in `undoStack`
   and `redoStack`.

## Timeline invalidation rule

When a new action occurs in one slice, clear `future` for other undoable slices
with pending futures. This prevents invalid branches across slices.
...
- Keep slice names consistent between reducer keys, middleware checks, and
  `UndoRedoType`.

## Adding a new undoable slice

1. Wrap reducer with `undoable(...)` and `historyEraser(...)` in
   `src/app/store.ts`.
